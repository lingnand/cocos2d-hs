...
import Linear.V2
import Prelude (
 ($), (*), (.), (/), (/=), (=<<), (==), (>), Double, Float, Maybe(..), fromIntegral, otherwise,
 recip, return, round
  )
...
#if MIN_VERSION_base(4,8,0)
instance {-# OVERLAPPING #-} Color3BValue (Colour Double) where
#else
instance Color3BValue (Colour Double) where
#endif
...
instance HoppyFHR.Encodable Color3B (Colour Double) where
  encode =
    \c -> let RGB r g b = toSRGB24 c in color3B_newFromRGB r g b

instance HoppyFHR.Encodable Color3BConst (Colour Double) where
  encode = HoppyP.fmap (toColor3BConst) . HoppyFHR.encodeAs (HoppyP.undefined :: Color3B)

instance HoppyFHR.Decodable Color3B (Colour Double) where
  decode = HoppyFHR.decode . toColor3BConst

instance HoppyFHR.Decodable Color3BConst (Colour Double) where
  decode =
...
#if MIN_VERSION_base(4,8,0)
instance {-# OVERLAPPING #-} Color4BValue (AlphaColour Double) where
#else
instance Color4BValue (AlphaColour Double) where
#endif
...
instance HoppyFHR.Encodable Color4B (AlphaColour Double) where
  encode =
    \ac -> do
      let
        a = alphaChannel ac
...
instance HoppyFHR.Encodable Color4BConst (AlphaColour Double) where
  encode = HoppyP.fmap (toColor4BConst) . HoppyFHR.encodeAs (HoppyP.undefined :: Color4B)

instance HoppyFHR.Decodable Color4B (AlphaColour Double) where
  decode = HoppyFHR.decode . toColor4BConst

instance HoppyFHR.Decodable Color4BConst (AlphaColour Double) where
  decode =
    \raw -> do
